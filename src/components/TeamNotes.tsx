...
const TeamNotes: React.FC<TeamNotesProps> = ({ notes, onAddNote }) => {
  ...
  return (
    <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/20 p-4 sm:p-6">
      ...
      {isAddingNote && (
        <form onSubmit={handleSubmit} className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg border border-gray-200/60 dark:border-gray-700/60">
          {/* User Info Display */}
          {user && (
            <div className="mb-3 sm:mb-4 p-3 bg-blue-50/80 dark:bg-blue-900/30 rounded-lg border border-blue-200/60 dark:border-blue-800/60">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-blue-100 dark:bg-blue-800 rounded-full flex items-center justify-center">
                  <span className="text-blue-700 dark:text-blue-300 font-bold text-xs">{user.initials}</span>
                </div>
          ...
          <div className="mb-3 sm:mb-4">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Note
              {category === 'summary' && (
                <span className="text-indigo-600 dark:text-indigo-400 font-medium"> (This will appear in Vehicle Notes section)</span>
              )}
            </label>
            <textarea
              value={newNote}
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            <button
              ...
              className="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm disabled:bg-gray-300 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
            >
              Add Note
            </button>
            <button
              ...
              className="px-3 sm:px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-400 dark:hover:bg-gray-500 transition-colors font-medium text-sm"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="space-y-3 sm:space-y-4">
        {notes.length === 0 ? (
          <div className="text-center py-6 sm:py-8 text-gray-500 dark:text-gray-400">
            <MessageSquare className="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2 text-gray-400 dark:text-gray-500" />
              ...
              return (
                <div key={note.id} className={`border rounded-lg p-3 sm:p-4 transition-colors ${
                  note.category === 'summary' 
                    ? 'border-indigo-200/60 dark:border-indigo-800/60 bg-indigo-50/50 dark:bg-indigo-900/20 hover:bg-indigo-50/80 dark:hover:bg-indigo-900/30' 
                    : 'border-gray-200/60 dark:border-gray-700/60 hover:bg-gray-50/50 dark:hover:bg-gray-800/50'
                }`}>
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3 mb-2">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                      <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                        note.category === 'summary' ? 'bg-indigo-100 dark:bg-indigo-800' : 'bg-blue-100 dark:bg-blue-800'
                      }`}>
                        <User className={`w-3 h-3 sm:w-4 sm:h-4 ${
                          note.category === 'summary' ? 'text-indigo-600 dark:text-indigo-300' : 'text-blue-600 dark:text-blue-300'
                        }`} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-gray-900 dark:text-gray-100 text-sm sm:text-base">{note.userInitials}</span>
                          {note.isCertified && (
                            <div className="flex items-center gap-1 px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full text-xs font-medium border border-green-200 dark:border-green-800">
                    ...
                    {note.category && (
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border flex-shrink-0 ${getCategoryColor(note.category)} dark:bg-opacity-30 dark:border-opacity-50`}>
                  ...
                  <p className={`text-sm sm:text-base ml-8 sm:ml-11 ${
                    note.category === 'summary' ? 'text-indigo-900 dark:text-indigo-200 font-medium' : 'text-gray-700 dark:text-gray-300'
                  }`}>
                    {note.text}
                  </p>
                  {note.category === 'summary' && (
